...
export default function App() {
  ...
  if (activeWorkout) return (
    <div className="min-h-screen bg-slate-50 pb-10">
      <div className="bg-blue-600 p-4 text-white flex justify-between items-center sticky top-0 z-10 shadow-lg">
  ...
  <div className="min-h-screen bg-slate-200 flex justify-center items-start md:py-8 font-sans">
    ...
    <div className="w-full max-w-md min-h-screen md:min-h-[850px] md:max-h-[90vh] bg-slate-50 shadow-2xl relative overflow-y-auto md:rounded-[3rem] border-x border-gray-200 flex flex-col">
      ...
      {activeTab === 'training' && (
        <>
          <header className="bg-white p-6 border-b shadow-sm sticky top-0 z-10 rounded-b-[2rem]">
          ...
          <div className="p-4 space-y-4">
            {data.filter(w => w.week === activeWeek).map(w => (
              <div key={w.id} onClick={() => setActiveWorkout(JSON.parse(JSON.stringify(w)))} className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-50 flex justify-between items-center group active:scale-95 transition-all">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-xs text-slate-400 mb-8 leading-relaxed">Verwalte deine Prompts, dein Equipment und erstelle Backups für deine Trainingsdaten.</p>
              <div className="grid grid-cols-2 gap-3">
                ...
                <button onClick={exportData} className="bg-blue-600 hover:bg-blue-500 p-5 rounded-3xl text-[10px] font-black uppercase tracking-widest flex flex-col items-center gap-3 transition-colors col-span-2"><Download size={24}/> Backup erstellen</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <nav className="fixed bottom-8 left-6 right-6 bg-white/90 backdrop-blur-2xl border border-white/20 shadow-[0_20px_50px_rgba(0,0,0,0.1)] rounded-[2.5rem] p-2 flex justify-between z-50">
        {[
